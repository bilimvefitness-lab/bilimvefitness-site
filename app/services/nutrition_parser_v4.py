...
class NutritionParserServiceV4:
    ...
    def _parse_segment(self, raw_segment: str, index: int) -> _ParsedSegment:
        ...
        portion = self._estimate_portion(food_match.food, amount, unit, all_modifiers)
        ...
        if needs_clarification and clarification_reason:
            question = ClarificationQuestion(
                ...
                suggested_options=self._clarification_options(food_match.food, portion.resolved_unit, all_modifiers),
            )
    ...
    def _estimate_portion(
        ...
        if amount is None:
            return PortionEstimate(None, None, None, False, unit, 0.0)

        resolved_unit = unit or food.default_unit
        estimated = unit is None and food.default_unit != "adet"

        if resolved_unit == "gram":
            return PortionEstimate(amount, amount, amount, False, resolved_unit, 0.0)
        if resolved_unit == "kilo":
            weight = amount * 1000
            return PortionEstimate(weight, weight, weight, False, resolved_unit, 0.0)
