...
from app.services.nutrition_food_catalog import FoodCatalog


class NutritionCalculationServiceV2:
            ...
            if not item.canonical_food_id or item.estimated_weight_g is None:
                ...
                if food is None:
                    ...
                    if item.needs_clarification:
                        calculation_basis = f"{round(item.estimated_weight_g, 2)} g üzerinden tahmini hesaplandı. Kaydetmeden önce doğrula."
                    elif item.estimated:
                        calculation_basis = f"{round(item.estimated_weight_g, 2)} g üzerinden tahmini hesaplandı."
                    else:
                    ...
                    if item.needs_clarification and clarification_reason:
                        clarification_questions.append(
                            ClarificationQuestion(
                                ...
                                raw_text=item.raw_text,
                                question=clarification_reason,
                                suggested_options=["küçük", "orta", "büyük", "gram"],
                            )
                        )
